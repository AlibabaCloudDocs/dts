...
-   如果同步的目标为云原生数据仓库AnalyticDB MySQL（2.0），那么源PolarDB-X中待同步的库或列的名称不能包含云原生数据仓库AnalyticDB MySQL（2.0）的保留字，否则将造成数据同步失败，详情请参见[保留字](https://help.aliyun.com/document_detail/98800.html)。
...
7.  单击页面右下角的**授权白名单并进入下一步**。
    ...
    **说明：** 此步骤会将DTS服务器的IP地址自动添加到PolarDB-X和云原生数据仓库AnalyticDB MySQL的白名单中，用于保障DTS服务器能够正常连接源实例和目标集群。
