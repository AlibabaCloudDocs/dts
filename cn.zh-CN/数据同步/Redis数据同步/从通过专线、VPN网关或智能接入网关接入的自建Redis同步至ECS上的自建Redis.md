...
数据传输服务DTS（Data Transmission Service）支持Redis数据库的单向同步，适用于异地多活、数据异地容灾等多种应用场景。本文以通过专线、VPN网关或智能接入网关接入的自建Redis同步至ECS上的自建Redis为例，介绍数据同步作业的配置流程。
...
-   源Redis数据库的版本为2.8、3.0、3.2、4.0或5.0版本。
    ...
    **说明：** 目标Redis数据库支持的版本为2.8、3.0、3.2、4.0或5.0版本，如需跨版本同步（仅支持从低版本同步到高版本）请提前确认兼容性。
...
6.  配置数据同步的源实例及目标实例信息。
    ...
    ![Redis集群同步源目实例信息配置](https://static-aliyun-doc.oss-accelerate.aliyuncs.com/assets/img/zh-CN/9249459951/p46325.png)
