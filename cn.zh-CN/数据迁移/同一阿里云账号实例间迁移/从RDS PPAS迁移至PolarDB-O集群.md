...
-   如需执行增量数据迁移，您需要[提交工单](https://selfservice.console.aliyun.com/ticket/category/rds/today)，为源RDS PPAS实例用于数据迁移的账号开通superuser权限。
...
-   RDS PPAS实例的账号创建及授权，请参见[创建账号](/cn.zh-CN/RDS PPAS 数据库/账号/创建账号.md)。

    **说明：** 如需执行增量数据迁移，您需要[提交工单](https://selfservice.console.aliyun.com/ticket/category/rds/today)，为该数据库账号申请开通superuser权限。
...
5.  配置迁移任务的源库和目标库连接信息。
    ...
    |实例地区|当实例选择为**有公网IP的自建数据库**时，**实例地区**无需设置。 **说明：** 单击**获取DTS IP段**来获取到DTS服务器的IP地址，并将获取到的IP地址加入RDS PPAS的IP白名单中，详情请参见[设置白名单](/cn.zh-CN/RDS PPAS 数据库/快速入门/设置白名单.md)。 |
    |数据库类型|选择**PPAS**。|
