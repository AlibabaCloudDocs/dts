...
Data Transmission Service \(DTS\) supports one-way data synchronization between Redis databases. This feature is applicable to scenarios such as active geo-redundancy and geo-disaster recovery. This topic describes how to configure one-way data synchronization from a user-created Redis database hosted on ECS to an ApsaraDB for Redis instance.
...
6.  Configure the source and destination instances.
    ...
    ![Configure the source and destination instances](https://static-aliyun-doc.oss-accelerate.aliyuncs.com/assets/img/en-US/2928449951/p46417.png)
    ...
    |ECS Instance ID|Select the ID of the ECS instance that is connected to the source Redis database. **Note:** If the source Redis database is deployed in a cluster architecture, select the ID of the ECS instance where a master node resides. |
    ...
    |Port Number|Enter the service port number of the source Redis database. The default port number is **6379**. In this example, enter **7000**. **Note:** If the source Redis database is deployed in a cluster architecture, enter the service port number of a master node. |
    ...
    |Database Password|Enter the database password of the destination ApsaraDB for Redis instance. **Note:** The format of the database password is <user\>:<password\>. For example, if the username of a custom account is admin and the password is Rp829dlwa, the database password is admin:Rp829dlwa. |
