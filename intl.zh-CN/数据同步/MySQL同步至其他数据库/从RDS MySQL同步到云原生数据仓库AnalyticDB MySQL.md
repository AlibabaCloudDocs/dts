...
由于MySQL和云原生数据仓库AnalyticDB MySQL的数据类型并不是一一对应的，所以DTS在进行结构初始化时，会根据数据类型定义进行类型映射，详情请参见[结构初始化涉及的数据类型映射关系](/intl.zh-CN/数据同步/结构初始化涉及的数据类型映射关系.md)。
...
6.  配置同步通道的源实例及目标实例信息。
    ...
    |数据库账号|填入云原生数据仓库AnalyticDB MySQL的数据库账号，权限要求请参见[数据库账号的权限要求](#section_w01_xz7_hp4)。|
    |数据库密码|填入该数据库账号对应的密码。|
...
1.  在云原生数据仓库AnalyticDB MySQL中创建一个新表（customer\_new），表结构与customer表保持一致。
