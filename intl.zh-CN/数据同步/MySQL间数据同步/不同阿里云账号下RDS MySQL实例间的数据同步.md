...
-   数据同步的源RDS实例和目标RDS实例已存在，如不存在请[创建RDS实例](https://www.alibabacloud.com/help/zh/doc-detail/26117.htm)。
...
将目标RDS实例所属的云账号设置为授信云账号，允许通过数据传输服务访问源RDS实例所属云账号的相关云资源。详情请参见[跨阿里云账号数据迁移或同步时如何配置RAM授权](/intl.zh-CN/RAM授权管理/跨阿里云账号数据迁移或同步时如何配置RAM授权.md)。

**说明：** 请使用源实例所属的阿里云账号登录RAM控制台执行授权操作，授权操作完成后，请在目标实例所属的阿里云账号中创建数据迁移或同步任务。

## 操作步骤

1.  使用目标RDS实例所属的阿里云账号[购买数据同步作业]()。

    **说明：** 购买时，选择源实例和目标实例均为**MySQL**，并选择同步拓扑为**单向同步**。

2.  使用目标RDS实例所属的阿里云账号登录[数据传输控制台](https://dts-intl.console.aliyun.com/)。
3.  在左侧导航栏，单击**数据同步**。
...
6.  配置同步通道的源实例及目标实例信息。
    ...
    ![MySQL单向同步源目实例信息配置](https://static-aliyun-doc.oss-accelerate.aliyuncs.com/assets/img/zh-CN/6420649951/p46227.png)
...
![选择其他阿里云账号下的RDS实例](https://static-aliyun-doc.oss-accelerate.aliyuncs.com/assets/img/zh-CN/6420649951/p46232.png) |
    |角色名称|填入[准备工作](#section_rdl_qrn_qhb)步骤中配置的RAM角色名称。|
    |RDS实例ID|选择源实例ID。|
