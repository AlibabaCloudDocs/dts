...
-   如果目标库的数据逐出策略（`maxmemory-policy`）配置为`noeviction`以外的值，可能导致目标库的数据与源库不一致。关于数据逐出策略详情，请参见[Redis数据逐出策略介绍](/intl.zh-CN/用户指南/常见问题/过期策略/ApsaraDB for Redis默认的数据逐出策略是什么？.md)。

## 支持的同步场景

**说明：**

-   下表中的数据库和架构支持任意组合，例如从云数据库Redis社区版（标准架构）同步至云数据库Redis企业版（集群架构）。
-   如需同步至不同类型或架构的实例，请先确认目标实例的相关命令限制不会影响您的业务。更多信息，请参见[Redis命令支持概览](/intl.zh-CN/产品简介/命令支持/Redis命令支持概览.md)。

|支持的源库|支持的目标库|支持的架构|支持的同步拓扑|
|-----|------|-----|-------|
|-   云数据库Redis社区版
...
|-   [标准（主从）架构](/intl.zh-CN/产品简介/部署架构/标准版-双副本.md)
-   [集群架构](/intl.zh-CN/产品简介/部署架构/集群版-双副本.md)
-   [读写分离架构](/intl.zh-CN/产品简介/部署架构/读写分离版.md)

|-   一对一单向同步
-   一对多单向同步
-   级联单向同步

更多信息，请参见[数据同步拓扑介绍](/intl.zh-CN/数据同步/数据同步拓扑介绍.md)。|
